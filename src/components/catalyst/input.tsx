...
export function InputGroup({ className, children }: { className?: string; children: React.ReactNode }) {
  return (
    <div className={cn(
      'relative',
      '[&>[data-slot=icon]:first-child]:absolute [&>[data-slot=icon]:first-child]:left-3 [&>[data-slot=icon]:first-child]:top-1/2 [&>[data-slot=icon]:first-child]:-translate-y-1/2 [&>[data-slot=icon]:first-child]:size-4 [&>[data-slot=icon]:first-child]:text-fg-subtlest [&>[data-slot=icon]:first-child]:pointer-events-none',
      '[&>[data-slot=icon]:first-child+input]:pl-9 [&>[data-slot=icon]:first-child+input]:sm:pl-8',
      '[&>input+[data-slot=icon]]:absolute [&>input+[data-slot=icon]]:right-3 [&>input+[data-slot=icon]]:top-1/2 [&>input+[data-slot=icon]]:-translate-y-1/2 [&>input+[data-slot=icon]]:size-4 [&>input+[data-slot=icon]]:text-fg-subtlest [&>input+[data-slot=icon]]:pointer-events-none',
      className,
    )}>
      {children}
    </div>
