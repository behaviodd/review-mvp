...
| **B2** | upsertRow / patchRow 의 동시성 | row 수정 시 다른 사용자의 동시 수정을 손실하지 않음 (CAS or merge) | ⚠️ **부분 해소 (라운드 14)** — `useStaleFormGuard` hook + MemberEditDialog 에 demo 적용. 폼 열린 동안 store row 가 변경되면 사용자에게 안내 + 새로 불러오기 액션. 다른 폼 (CycleEdit, CycleSettingsDrawer, MemberAddDialog 등) 은 후속 phase 에서 동일 패턴 확장 | 미적용 폼은 여전히 last-write-wins. UI hook 일반화 완료라 적용 확장 비용 작음 | **P2** (잔여) |
...
| **B7** | `markWrite` grace 4s 의 stale-overwrite 보호 | 자기 쓰기 직후 polling 이 stale 시트로 optimistic 상태를 덮어쓰지 않음 | ✅ **라운드 14 해소** — `BroadcastChannel('sheets-sync')` 로 다중 탭에 markWrite 공유. 받은 탭은 `lastWriteAt` 갱신 (더 최신 timestamp 만). 단일 운영자 다탭 시 다른 탭의 polling 도 grace 적용 | (다른 사용자의 쓰기는 여전히 grace 모름 — broadcast 채널이 동일 origin·동일 브라우저 한정. 외부 시연·운영 다인 환경은 향후 server push 필요) | ~~P2~~ |
