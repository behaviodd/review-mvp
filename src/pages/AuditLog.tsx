...
export function AuditLog() {
  ...
  useSetPageHeader('감사 로그', undefined, {
    subtitle: `마스터 로그인 ${impersonationLogs.length}건 · 시스템 ${auditEntries.length}건`,
  });
  ...
  const tabs: { value: TabKey; label: string; count: number }[] = [
    { value: 'impersonation', label: '마스터 로그인', count: impersonationLogs.length },
    { value: 'system',        label: '시스템 액션',   count: auditEntries.length },
    { value: 'all',           label: '전체',           count: impersonationLogs.length + auditEntries.length },
  ];
      ...
      {(tab === 'impersonation' || tab === 'all') && (
        <section className="space-y-2">
          <header className="flex items-center gap-2">
            ...
            <h2 className="text-xs font-bold uppercase tracking-wider text-gray-050">마스터 로그인</h2>
            {impLoading && <span className="text-[11px] text-gray-040">로딩 중…</span>}
          </header>
                ...
                return (
                  <div key={log.id} className="flex items-center gap-3 px-5 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {log.ip && (
                        <p className="text-[10px] text-gray-030 mt-0.5 truncate">IP: {log.ip}</p>
                      )}
                    </div>
                    <span className="text-[11px] text-gray-040 shrink-0">{timeAgo(log.startedAt)}</span>
                  </div>
                );
              })}
            </div>
      ...
      {(tab === 'system' || tab === 'all') && (
        <section className="space-y-2">
          <header className="flex items-center gap-2">
            ...
            <h2 className="text-xs font-bold uppercase tracking-wider text-gray-050">시스템 액션</h2>
          </header>
                  ...
                  <div key={entry.id} className="flex items-center gap-3 px-5 py-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-gray-080">
                        ...
                        <span className="ml-1 text-gray-050">— {entry.summary}</span>
                      </p>
                      <p className="text-[11px] text-gray-040 mt-0.5">
                    ...
                    <span className="text-[11px] text-gray-040 shrink-0">{timeAgo(entry.at)}</span>
                  </div>
                );
              })}
              {auditEntries.length > 200 && (
                <div className="px-5 py-3 text-center text-[11px] text-gray-040">
                  최근 200개만 표시됩니다.
                </div>
              )}
            </div>
