...
| **B5** | `batchUpsertUsers` / `batchUpsertOrgUnits` 부분 실패 | row N개 upsert 가 모두 성공하거나 client 가 어디까지 반영됐는지 알 수 있음 | ❌ **rows.forEach** 도중 throw → 1·2 는 시트 반영, 3·4·5 미반영. response = `{error}` 이고 partial state 미가시화 | client 가 retry 하면 1·2 는 두 번 반영(멱등이라 OK), 그러나 client 의 optimistic 상태가 partial state 와 어긋남 | **P1** |
| **B6** | `deleteCycle` 의 cascade 정합성 | cycle row 삭제 시 모든 submissions 도 함께 삭제 (또는 둘 다 안 됨) | ⚠️ **순차 삭제** — cycle row 삭제 후 submissions loop. submission 삭제 도중 throw 시 cycle 만 사라지고 submissions orphan | 사이클 삭제 도중 Apps Script 6분 한계 또는 transient 오류 → orphan submissions 잔재. cycle 재생성 시 옛 submissions 이 ghost 로 표시될 가능성 | **P1** |
...
| **B9** | redirect:'manual' POST 처리 | 302 location 추출 + 실제 doPost 호출 보장 | ⚠️ **edge case 2건**: (a) `location` null 시 302 본문 그대로 전달 → JSON parse 실패. (b) 두 번째 fetch 가 첫 fetch 의 timer 와 같은 `ctrl` 사용 → **남은 시간만 timeout** | 첫 fetch 에 17초 → 두 번째 fetch 는 1초만 가능 → cold start 시 추가 abort 빈도 높음. 본인 retry 안 하니 사용자에게 timeout 노출 | **P1** |
