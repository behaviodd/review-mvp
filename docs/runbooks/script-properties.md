...
review-mvp 의 Apps Script(canonical: Apps Script 편집기 / 로컬 사본: `~/Desktop/apps-script/ReviewFlow.gs`)는 다음 시크릿/설정을 **Script Properties** 에서 읽는다. **코드에 fallback 을 두지 않는다.** 누락되면 `verifyGoogleIdToken_` 등이 즉시 에러를 반환한다.
...
- 코드 위치: Apps Script 편집기 (canonical) / 로컬 사본 `~/Desktop/apps-script/ReviewFlow.gs` 의 `verifyGoogleIdToken_()`
